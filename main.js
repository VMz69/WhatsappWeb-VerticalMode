/**
 * @file clean_main.js
 * @description Modulo principal de Electron sanitizado y refactorizado.
 */
...
const createWindow = () => {
    ...
    win.loadURL("https://web.whatsapp.com").catch((error) => {
    ...
    const applyHack = () => {
        ...
        win.webContents.executeJavaScript(`
            (() => {
                ...
                appNode.style.flexDirection = 'row';
                appNode.style.width = '200%';
                ...
                const enableHackLayout = () => {
                    appNode.style.display = 'flex';
                    appNode.style.flexDirection = 'row';
                    appNode.style.width = '200%';
                    if (appNode.children) {
                        for (const child of appNode.children) {
                            child.style.width = '100%';
                            child.style.flexShrink = '0';
                        }
                    }
                ...
                const disableHackLayout = () => {
                    appNode.style.display = '';
                    appNode.style.flexDirection = '';
                    appNode.style.width = '';
                    if (appNode.children) {
                        for (const child of appNode.children) {
                            child.style.width = '';
        ...
        `).catch(err => console.error("Error executing hack JS:", err));
    };
    ...
    win.webContents.on('did-finish-load', () => {
        const interval = setInterval(async () => {
            try {
                ...
                console.error("Error checking for main element:", error);
                clearInterval(interval);
            }
        }, 1000);
    });
...
app.whenReady().then(createWindow).catch(error => console.error("App ready error:", error));
